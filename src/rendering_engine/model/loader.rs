...
impl ObjLoader {
    ...
    fn parse_raw_vertex(input: &str) -> [f32; 3] {
        let values: Vec<f32> = input
            ...
            .map(|v| {
                v.parse::<f32>()
                    .unwrap_or_else(|_| panic!("invalid values for a vertex: {}", input))
            })
            .collect();
        [
...
struct RawFace {
    ...
    pub _text_indices: Option<[usize; 3]>,
}
impl RawFace {
    pub fn new(input: &str, invert_winding: bool) -> Self {
        ...
        Self {
            ...
            _text_indices: RawFace::parse(arguments.clone(), 1, invert_winding),
        }
    }

    fn parse(inputs: Vec<&str>, index: usize, invert_winding: bool) -> Option<[usize; 3]> {
        let f1: Vec<&str> = inputs.first().unwrap().split('/').collect();
        let f2: Vec<&str> = inputs.get(1).unwrap().split('/').collect();
        let f3: Vec<&str> = inputs.get(2).unwrap().split('/').collect();
        let a1 = *f1.get(index).unwrap();
        let a2 = *f2.get(index).unwrap();
        let a3 = *f3.get(index).unwrap();
        match a1 {
            "" => None,
            _ => {
